...
import { createWorkService, getWorksService } from "../services/work.service";
import type { CreateWorkInput } from "../type";
...
export async function createWorkController(req: Request, res: Response) {
  try {
    ...
    const result = await createWorkService(
...
export async function getWorksController(req: Request, res: Response) {
  try {
    const result = await getWorksService();
